...
class CatalogAPI {
  ...
  Future<ProductsResponse> getProductsOfCategory(int cateId, int page) async {
    final pageSize = 20;
    final params = <String, String>{'fields': 'sku'};
    var uri = Uri.parse(ClientConfigs.baseURL +
        ClientConfigs.requestType +
        ClientConfigs.storeID +
        ClientConfigs.version +
    ...
    final response1 = await http.get(uri, headers: {'Authorization': 'Bearer ' + ClientConfigs.accessToken});
  ...
  Future<ProductsResponse> getProductsWithSKUs(List<String> skus, int pageSize, int page) async {
    ...
    print(uri);
    final response2 = await http.get(uri, headers: {'Authorization': 'Bearer ' + ClientConfigs.accessToken});
